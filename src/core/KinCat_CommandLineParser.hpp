#ifndef __KINCAT_COMMAND_LINE_PARSER_HPP__
#define __KINCAT_COMMAND_LINE_PARSER_HPP__
...
template <> Option<float>::Option(std::string desc, float val) : _desc(desc), _type("float") {
  std::ostringstream out;
  out << std::scientific << std::setprecision(15) << val;
  _val = out.str();
}
template <> Option<double>::Option(std::string desc, double val) : _desc(desc), _type("double") {
  std::ostringstream out;
  out << std::scientific << std::setprecision(15) << val;
  _val = out.str();
}
  ...
  void print_option(const std::string argv0) {
    ...
    for (auto it = _map.begin(); it != _map.end(); ++it) {
      ...
      if (std::get<1>(val) == "bool") {
        ...
        std::cout << std::setw(42) << " "
                  << "(default: " << key << "=" << std::scientific << std::get<2>(val) << ")\n";
      }
    }
                ...
                if (tval != NULL) {
                  if (type == "bool") {
                    *((bool *)tval) = true;
                    ...
                    printf("   parsing type %s\n", type.c_str());
                    std::runtime_error("Error: missing rhs value where the type is not bool");
                  }
