...
ProcessCounter<DT>::ProcessCounter(const ordinal_type n_processes,
                                   const value_type_2d_view<ordinal_type, DT> &processints)
    : _is_initialized(false), _processints(processints), _n_processes(n_processes), _process_counter(),
...
template <typename DT> void ProcessCounter<DT>::initialize(const ordinal_type n_samples) {
  _process_counter = value_type_2d_view<size_type, DT>("process counter", n_samples, _n_processes);
  _processints_counter =
      value_type_2d_view<size_type, DT>("process instance counter", n_samples, _processints.extent(0) + 1);
  ...
  if (_is_initialized) {
    ...
    for (ordinal_type s = 0, send = n_samples; s < send; ++s) {
      ...
      for (ordinal_type i = 0, iend = _process_counter_host.extent(1); i < iend; ++i)
        os << indent4 << "[ " << i << ", " << _process_counter_host(s, i) << "] \n";

      if (verbose > 3) {
        os << indent2 << "-- process counter : \n";
        for (ordinal_type i = 0, iend = (_processints_counter_host.extent(1) - 1); i < iend; ++i)
          os << indent4 << "[ " << i << ", " << _processints_counter_host(s, i) << "] \n";
      }
    }
  }
  return os;
